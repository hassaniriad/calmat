...
!   
   ...
   integer  (Ikind), private, parameter   :: NFC = 101, & ! number of usual functions
                                             NOP = 32     ! number of operators
                                              
   type     (str_t), public , save        :: FuncList(NFC), & ! List of function names
                                             FuncDesc(NFC), & ! Their short descriptions
                                             OperList(NOP), & ! list of operators names
                                             OperDesc(NOP)    ! Their short descriptions
   integer  (Ikind), public , save        :: OperCode(NOP)    ! Operator codes
...
!        S01, S02 and SO3: used to mark a given character in the expression
   ...
   character(len=1), parameter :: SOP = achar(192), SCL = achar(193), &
                                  S01 = achar(194), S02 = achar(195), S03 = achar(196)    
!
! And the following will be used in place of operators that have more than one character:
!
   integer  (Ikind), private, parameter :: iELEMMUL   = 131, iELEMDIV   = 132, & 
                                           iELEMPOW   = 133, iEQUAL     = 134, & 
                                           iNOTEQ     = 135, iLESSOREQ  = 136, & 
                                           iGREATOREQ = 137, iELEMTRANS = 138    

   character(len=1), private, parameter :: ELEMMUL   = achar(iELEMMUL)  , & ! used for .*
                                           ELEMDIV   = achar(iELEMDIV)  , & !          ./
                                           ELEMPOW   = achar(iELEMPOW)  , & !          .^
                                           EQUAL     = achar(iEQUAL)    , & !          ==
                                           NOTEQ     = achar(iNOTEQ)    , & !          ~=
                                           LESSOREQ  = achar(iLESSOREQ) , & !          <=
                                           GREATOREQ = achar(iGREATOREQ), & !          >=
                                           ELEMTRANS = achar(iELEMTRANS)    !          .'
...
!=============================================================================================
   SUBROUTINE pk2Interpreter_TokensAnalyzer ( myExpr, myVarNames, handle, flagerr ) ! modified (riad)
!=============================================================================================
...
!- local variables used/modified by the internal procedures of pk2Interpreter_TokensAnalyzer--
   ...
   integer  (Ikind)              :: opAddSub(4), opMulDiv(7), opPower(2), opOr(1), opAnd(1), &
                                    opRelat(6)
...
!      
   ...
   !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
      RECURSIVE SUBROUTINE pk2Interpreter_Or ( ) ! Added (riad)
   !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
   ...
   !------------------------------------------------------------------------------------------  
      ...
      do while ( toke == '|' )
         ...
         if ( opOr(isOr-1) == operCode(3) ) then
   ...
   !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
      RECURSIVE SUBROUTINE pk2Interpreter_And ( ) ! Added (riad)
   !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
   ...
   !------------------------------------------------------------------------------------------  
      ...
      do while ( toke == '&' ) 
         ...
         if ( opAnd(isAnd-1) == operCode(10) ) then
   ...
   !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
      RECURSIVE SUBROUTINE pk2Interpreter_Relat ( ) ! Added (riad)
   !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
   ...
   !------------------------------------------------------------------------------------------  
      ...
      do while ( toke == '<' .or. toke == LESSOREQ  .or. toke == EQUAL .or. &
         ...
         if ( opRelat(isRelat-1) == operCode(14)) then
            ...
            !case ('~=') 
         else if ( opRelat(isRelat-1) == operCode(15) ) then
               isRelat                           = isRelat - 1
               handle%operations(handle%noperat) = 15
               handle%noperat                    = handle%noperat + 1            

         else if ( opRelat(isRelat-1) == operCode(16) ) then
               isRelat                           = isRelat - 1
               handle%operations(handle%noperat) = 16
               handle%noperat                    = handle%noperat + 1            

         else if ( opRelat(isRelat-1) == operCode(17) ) then
               isRelat                           = isRelat - 1
               handle%operations(handle%noperat) = 17
               handle%noperat                    = handle%noperat + 1            

            !case ('<=') 
         else if ( opRelat(isRelat-1) == operCode(18) ) then
               isRelat                           = isRelat - 1
               handle%operations(handle%noperat) = 18
               handle%noperat                    = handle%noperat + 1            

            !case ('>=') 
         else if ( opRelat(isRelat-1) == operCode(19) ) then
               isRelat                           = isRelat - 1
   ...
   !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
      RECURSIVE SUBROUTINE pk2Interpreter_AddSub ( ) ! adapted (riad)
   !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
   ...
   !------------------------------------------------------------------------------------------  
      ...
      do while ( toke == '+' .or. toke == '-' )
         ...
         if ( opAddSub(isAddSub) == operCode(1) ) then
      
               handle%operations(handle%noperat) = 1
               handle%noperat                    = handle%noperat + 1

         else if ( opAddSub(isAddSub) == operCode(2) ) then
   ...
   !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
      RECURSIVE SUBROUTINE pk2Interpreter_MulDiv ( ) ! adapted (riad)
   !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
   ...
   !------------------------------------------------------------------------------------------  
      ...
      do while ( toke == '*' .or. toke == ELEMMUL .or. &
         ...
         if ( opMulDiv(isMulDiv) == operCode(6) ) then
               handle%operations(handle%noperat) = 6
               handle%noperat                    = handle%noperat + 1 
                  
         else if ( opMulDiv(isMulDiv) == operCode(7) )then
               handle%operations(handle%noperat) = 7
               handle%noperat                    = handle%noperat + 1

         else if ( opMulDiv(isMulDiv) == operCode(8) )then
               handle%operations(handle%noperat) = 8
               handle%noperat                    = handle%noperat + 1          
            
         else if ( opMulDiv(isMulDiv) == operCode(9) )then
               handle%operations(handle%noperat) = 9
               handle%noperat                    = handle%noperat + 1                           

         else if ( opMulDiv(isMulDiv) == operCode(11) )then
   ...
   !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
      RECURSIVE SUBROUTINE pk2Interpreter_Unary ( ) ! adapted (riad)
   !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
   ...
   !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
      RECURSIVE SUBROUTINE pk2Interpreter_Pow ( ) ! adapted (riad)
   !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
   ...
   !------------------------------------------------------------------------------------------
      ...
      do while ( toke == "^" .or. toke == ELEMPOW .or. & ! ^ or .^
         ...
         opPower(isPower) = iachar(toke) ; isPower = isPower + 1
         
         itoke = itoke + 1 ; toke = handle%stokens(itoke)%str
                      
         if ( opPower(isPower-1) == operCode(12) ) then
         ...
         else if ( opPower(isPower-1) == operCode(13) )  then
         ...
         else if ( opPower(isPower-1) == operCode(28) )  then

               isPower                           = isPower - 1    
               handle%operations(handle%noperat) = 28
               handle%noperat                    = handle%noperat + 1    

         else if ( opPower(isPower-1) == operCode(29) )  then
   ...
   !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
      RECURSIVE SUBROUTINE pk2Interpreter_Functions ( ) ! modified (riad)
   !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
   ...
   !
            Id = pk2Interpreter_FindId ( toke, FuncList )
   ...
   !
            Id = pk2Interpreter_FindId ( toke, myVarNames )   
            ...
            if ( Id /= 0 ) then
               handle%operations (handle%noperat) = NFC + NOP + Id
   ...
   !
                  call flagerr%set ( stat = UERROR, where ='pk2Interpreter_Functions', &
   ...
   !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
      RECURSIVE SUBROUTINE pk2Interpreter_Brackets ( ) ! adapted (riad)
   !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
   ...
   !     for multiple arguments (delimited by ";" or ","):
         ...
         if ( toke /= ')' ) then  
            call flagerr%set ( stat = UERROR, where = 'pk2Interpreter_Brackets', msg =  & 
         ...
         if ( toke == '(' ) then
            call flagerr%set ( stat = UERROR, where = 'pk2Interpreter_Brackets', &
   ...
   !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
      SUBROUTINE pk2Interpreter_RecogVars ( ) ! modified (riad)
   !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
   ...
   ! 
      Id = util_FindPosInUnsortedList ( OperCode, IONE, NOP, iachar(toke(1:1)) )      
      !Id = pk2Interpreter_FindId ( toke, OperCode )
      if ( Id /= 0 ) then   
          call flagerr%set ( stat = UERROR, where = 'pk2Interpreter_RecogVars', &
                              msg = 'Expression has an error'                   )
          return    
      end if 
   ...
   !     Set the operation # to (NFC + NOP) + Id:
   !   
         handle%operations (handle%noperat) = (NFC + NOP) + Id
         handle%involvedVar(handle%noperat) = myVarNames(Id) ! 04/24
   ...
   !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
      SUBROUTINE pk2Interpreter_UndefVar ( ) ! added (riad)
   !-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
   ...
   !------------------------------------------------------------------------------------------   
      ...
      if ( any(handle%operations(1:handle%noperat) < 0) ) then
         ...
         call flagerr%set ( stat = UERROR+10, where = 'pk2Interpreter_UndefVar', msg = msg )
      end if
...
!=============================================================================================    
   SUBROUTINE pk2Interpreter_EvalExpr_v1 ( handle, vars, answer, flagerr, nanswer ) ! modified (riad)
!============================================================================================= 
   type   (hdl_t),              intent(in out) :: handle
...
!        Another version (pk2Interpreter_EvalExpr_v3) is given below, I think more efficient
!        since it avoids assignments and corresponding temporary copies.
!
!  05/25: Version NO LONGER MAINTAINED (use pk2Interpreter_EvalExpr_v3)
!---------------------------------------------------------------------------------------------    
...
!---------------------------------------------------------------------------------------------    
   ...
   if (.not. handle%initialized) then
      call flagerr%set ( stat = UERROR, where = HERE,                   &
...
!   
      ...
      if (err /= 0) then
         call flagerr%set ( stat = IERROR, where = HERE, msg = 'Allocation failure' )  
         return
      end if         
      do i = 1, nsdata
         handle%sdata(i)%name = ''
...
!                              
         case ( NOP+NFC+1: )         
            ...
            if ( ivar > nvars .or. ivar <= 0 ) then
               call flagerr%set ( stat = IERROR, where = HERE, msg = 'Wrong variable Id' )
               return
...
!
               call flagerr%set ( stat = WARNING, where = HERE,              &
                                   msg = 'The list of variables has changed' )
               ivar = pk2Interpreter_FindVarId ( handle%involvedVar(i)%str, vars )
               if ( ivar == 0 ) then
                  call flagerr%set ( stat = UERROR, where = HERE, msg =                  &
                                    'Unknown variable "'//handle%involvedVar(i)%str//'"' )
                  return
               else
...
!                              
         case default
            call flagerr%set ( stat = IERROR, where = HERE, msg = HERE )
            return
      ...
      if ( nsused /= nsdata ) then
         ...
         flagerr%mesg = &
         "The number of outputs ("//trim(cnum)//") differs from the requested one (" 
...
! !=============================================================================================    
!    SUBROUTINE pk2Interpreter_EvalExpr_v2 ( handle, vars, answer, flagerr, nanswer ) ! modified (riad)
! !============================================================================================= 
!    type   (hdl_t),              intent(in out) :: handle
...
! !
! !  ...hum the life is not simple ... I am doing aliasing in this version, e.g. in
! !           call pk2f_subSIN (a=pdata(st), res=pdata(st) ) or in
! !           call pk2_subAdd  (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
! !     which is prohibited by the standard (even though I haven't noticed any problems with it)
! !
! !     A third version (pk2Interpreter_EvalExpr_v3) uses a local variable (res) and 
! !     then performs a move_allocation to the target (pdata(?))
! !---------------------------------------------------------------------------------------------    
! 
! !- local variables --------------------------------------------------------------------------- 
...
!             call pk2_subSub (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!             st = st - 1
! 
!          case ( 3 ) ! a | b (or)
!             call pk2_subOr (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!             st = st - 1
!       
!          case ( 4 ) ! -a (minus)
!             call pk2_subMinus (a=pdata(st), res=pdata(st))
!             
!          case ( 5 ) ! ~a (not)
!             call pk2_subNot (a=pdata(st), res=pdata(st))
!                         
!          case ( 6 ) !  a * b (matrix-matrix product)
!             call pk2_subMult (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!             st = st - 1
! 
!          case ( 7 ) ! a / b (divide: mat/scal or scal/mat (mat of inverses))
!             call pk2_subDiv (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!             st = st - 1                                            
! 
!          case ( 8 ) ! a .* b (element-wise mutliplication)
!             call pk2_subEMult (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!             st = st - 1
! 
!          case ( 9 ) ! a ./ b (element-wise divide)
!             call pk2_subEDiv (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!             st = st - 1 
!                         
!          case ( 10 ) ! a & b (and)
!             call pk2_subAnd (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!             st = st - 1       
! 
!          case ( 11 ) ! lin. syst. solve
!             call pk2_smldivide (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!             st = st - 1       
!             
!          case ( 12 ) ! a^b (exponentiation)
!             call pk2_subPow (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!             st = st - 1       
! 
!          case ( 13 ) ! a.^b (element-wise exponentiation)
!             call pk2_subEPow (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!             st = st - 1       
! !
! !-       relational operators:
! !
!          case ( 14 ) ! a == b
!             call pk2_subEq (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!             st = st - 1       
! 
!          case ( 15 ) ! a /= b 
!             call pk2_subNe (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!             st = st - 1       
! 
!          case ( 16 )  ! a < b
!             call pk2_subLt (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!             st = st - 1       
! 
!          case ( 17 ) ! a > b
!             call pk2_subGt (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!             st = st - 1       
! 
!          case ( 18 ) ! a <= b
!             call pk2_subLe (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!             st = st - 1       
! 
!          case ( 19 ) ! a >= b
!             call pk2_subGe (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!             st = st - 1       
! 
!          case ( 28 ) ! ' (conj. transpose)
!              call trans   (a=pdata(st), res=pdata(st), conj=.true.)
! 
!          case ( 29 ) ! .' (transpose)
!              call pk2f_subTRANS   (a=pdata(st), res=pdata(st), conj=.false.)
! 
! !
...
!             else
!                if      ( iop == NOP+ 7 ) then
!                   call pk2f_subSIN     (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+ 8 ) then
!                   call pk2f_subCOS     (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+ 9 ) then
!                   call pk2f_subTAN     (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+10 ) then
!                   call pk2f_subASIN    (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+11 ) then
!                   call pk2f_subACOS    (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+12 ) then
!                   call pk2f_subATAN    (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+13 ) then 
!                   call pk2f_subSIND    (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+14 ) then 
!                   call pk2f_subCOSD    (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+15 ) then 
!                   call pk2f_subTAND    (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+16 ) then 
!                   call pk2f_subASIND   (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+17 ) then 
!                   call pk2f_subACOSD   (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+18 ) then 
!                   call pk2f_subATAND   (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+19 ) then
!                   call pk2f_subSINH    (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+20 ) then
!                   call pk2f_subCOSH    (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+21 ) then
!                   call pk2f_subTANH    (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+22 ) then 
!                   call pk2f_subLOG     (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+23 ) then 
!                   call pk2f_subLOG10   (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+24 ) then 
!                   call pk2f_subEXP     (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+25 ) then 
!                   call pk2f_subSQRT    (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+26 ) then 
!                   call pk2f_subABS     (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+27 ) then 
!                   call pk2f_subERF     (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+28 ) then 
!                   call pk2f_subERFC    (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+29 ) then 
!                   call pk2f_subGAMMA   (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+30 ) then 
!                   call pk2f_subFACTOR  (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+31 ) then 
!                   call pk2f_subNINT    (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+32 ) then 
!                   call pk2f_subANINT   (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+33 ) then 
!                   call pk2f_subAINT    (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+34 ) then 
!                   call pk2f_subFLOOR   (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+35 ) then 
!                   call pk2f_subCEIL    (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+36 ) then 
!                   call pk2f_subSIGN    (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+37 ) then 
!                   call pk2f_subHEAV    (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+38 ) then 
!                   call pk2f_subREAL    (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+39 ) then 
!                   call pk2f_subIMAG    (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+40 ) then 
!                   call pk2f_subCONJ    (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+41 ) then 
!                   call pk2f_subTRACE   (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+42 ) then 
!                   call pk2f_subTRANS   (a=pdata(st), res=pdata(st), conj=.true.)
!                else if ( iop == NOP+43 ) then 
!                   call pk2f_subTRANS   (a=pdata(st), res=pdata(st), conj=.false.)
!                else if ( iop == NOP+44 ) then 
!                   call pk2f_subDET     (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+45 ) then 
!                   call pk2f_subINV     (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+47 ) then 
!                   call pk2f_subNUMEL   (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+48 ) then 
!                   call pk2f_subFIND    (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+49 ) then 
!                   call pk2f_subNUM2STR (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+50 ) then 
!                   call pk2f_subSTR2NUM (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+51 ) then 
!                   call pk2f_subMAGIC   (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+52 ) then 
!                   call pk2f_subISSYMM  (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+53 ) then 
!                   call pk2f_subISSKEW  (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+80 ) then 
!                   call pk2f_subTRIM    (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+81 ) then 
!                   call pk2f_subLENGTH  (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+46 ) then
! !
! !-                caution: this function has a variable number of outputs:
! !               
!                   if ( nsdata == 0 ) then
!                      call pk2f_subSIZE (a=pdata(st), res=pdata(st))
!                   else 
!                      call pk2f_subSIZE (a=pdata(st), res=pdata(st), res2=sdata(1))
!                      nsused = max(nsused,1_Ikind)
!                   end if                     
!                else if ( iop == NOP+68 ) then
! !
...
! !                              
!                   if ( nsdata == 0 ) then
!                      call pk2f_subEIG (a=pdata(st), eigval=pdata(st))
!                   else
!                      call pk2f_subEIG (a=pdata(st), eigval=pdata(st), eigvec=sdata(1))
!                      nsused = max(nsused,1_Ikind)
!                   end if
!                else if ( iop == NOP+89 ) then
! !
...
! !                                             
!                   if ( nsdata == 0 ) then
!                      call pk2f_subLU (a=pdata(st), res=pdata(st))
!                   else if ( nsdata == 1 ) then
!                      call pk2f_subLU (a=pdata(st), res=pdata(st), res2=sdata(1))
!                      nsused = max(nsused,1_Ikind)
!                   else
!                      call pk2f_subLU (a=pdata(st), res=pdata(st), res2=sdata(1), res3=sdata(2))
!                      nsused = max(nsused,2_Ikind)
!                   end if  
!                   
!                else if ( iop == NOP+96 ) then 
!                   call pk2f_subDEV (a=pdata(st), res=pdata(st))
!                else if ( iop == NOP+98 ) then 
!                   call pk2f_subSIZEOF (a=pdata(st), res=pdata(st))                  
!                else if ( iop == NOP+99 ) then 
!                   call pk2f_subTYPEOF (a=pdata(st), res=pdata(st))   
!                   
!                else if ( iop == NOP+100 ) then
! !
! !-                caution: this function has a variable number of outputs:
! !                                             
!                   if ( nsdata == 0 ) then
!                      call pk2f_subSVD (a=pdata(st), s=pdata(st))
!                   else if ( nsdata == 1 ) then
!                      call pk2f_subSVD (a=pdata(st), u=pdata(st), s=sdata(1))
!                      nsused = max(nsused,1_Ikind)
!                   else
!                      call pk2f_subSVD (a=pdata(st), u=pdata(st), s=sdata(1), v=sdata(2))
!                      nsused = max(nsused,2_Ikind)
!                   end if                 
!                   
!                else if ( iop == NOP+101 ) then
...
! !                                        
!                   if ( nsdata == 0 ) then
!                      call pk2f_subPoldec (F=pdata(st), R=pdata(st))
!                   else 
!                      call pk2f_subPoldec (F=pdata(st), R=pdata(st), U=sdata(1))
!                      nsused = max(nsused,1_Ikind)
!                   end if                 
! 
!                end if  
...
!                   call pk2_subGe         (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!                else if ( iop == NOP+55 ) then
!                   call pk2f_subCROSS     (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!                else if ( iop == NOP+57 ) then 
!                   call pk2f_subMLDIVIDE  (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!                else if ( iop == NOP+86 ) then 
!                   call pk2f_subMOD       (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!                else if ( iop == NOP+90 ) then 
!                   call pk2f_subLUSOLV    (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!                else if ( iop == NOP+92 ) then
! !
! !-                caution: this function has a variable number of outputs:
! !                 
!                   if ( nsdata == 0 ) then
!                      call pk2f_subREGLIN (x=pdata(st-1), y=pdata(st), res=pdata(st-1))
!                   else if ( nsdata == 1 ) then
!                      call pk2f_subREGLIN (x=pdata(st-1), y=pdata(st), res=pdata(st-1), &
!                                                                       b  =sdata(1   )  )
!                      nsused = max(nsused,1_Ikind)
...
!                   end if   
!                else if ( iop == NOP+95 ) then
!                   call pk2f_subPART      (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!                else if ( iop == NOP+97 ) then
!                   call pk2f_subINTTRAP   (a=pdata(st-1), b=pdata(st), res=pdata(st-1))
!                   
!                end if
!                
!                st = st - 1                         
...
!          case ( NOP+54 )
!             k0 = st-narg(i)+1
!             call pk2f_subNORMwrp     (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
! 
!          case ( NOP+56 )
!             k0 = st-narg(i)+1
!             call pk2f_subRESHAPEwrp  (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
!          
!          case ( NOP+58 )
!             k0 = st-narg(i)+1
!             call pk2f_subZEROSwrp    (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
! 
!          case ( NOP+59 )
!             k0 = st-narg(i)+1
!             call pk2f_subFALSESwrp   (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
! 
!          case ( NOP+60 )
!             k0 = st-narg(i)+1
!             call pk2f_subONESwrp     (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
!             
!          case ( NOP+61 )   
!             k0 = st-narg(i)+1
!             call pk2f_subEYEwrp      (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
! 
!          case ( NOP+62 )
!             k0 = st-narg(i)+1
!             call pk2f_subRANDwrp     (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
! 
!          case ( NOP+63 )
!             k0 = st-narg(i)+1
!             call pk2f_subDIAGwrp     (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
!             
!          case ( NOP+64 )
!             k0 = st-narg(i)+1
!             call pk2f_subTRILwrp     (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
!             
!          case ( NOP+65 )
!             k0 = st-narg(i)+1
!             call pk2f_subTRIUwrp     (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
! 
!          case ( NOP+66 )
!             k0 = st-narg(i)+1
!             call pk2f_subCOLONwrp    (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
!             
!          case ( NOP+67 )
!             k0 = st-narg(i)+1
!             call pk2f_subDATEwrp     (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
!                 
!          case ( NOP+69 )
!             k0 = st-narg(i)+1
!             call pk2f_subMERGEMATS   (matrs=[(pdata(k),k=k0,st)], res=pdata(k0), &
!                                       delim=handle%csep(i)%str)
!             st = k0
! 
!          case ( NOP+70 )    
!             k0 = st-narg(i)+1
!             call pk2f_subMINMAXwrp   (matrs=[(pdata(k),k=k0,st)], res=pdata(k0), &
!                                       minmax='min')
!             st = k0
! 
!          case ( NOP+71 )     
...
!          case ( NOP+72 )   
!             k0 = st-narg(i)+1
!             call pk2f_subSUMwrp      (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
! 
!          case ( NOP+73 )     
!             k0 = st-narg(i)+1
!             call pk2f_subMEANwrp     (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
! 
!          case ( NOP+74 )      
!             k0 = st-narg(i)+1
!             call pk2f_subPRODwrp     (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
! 
!          case ( NOP+77 )
!             k0 = st-narg(i)+1
...
!          case ( NOP+78 )
!             k0 = st-narg(i)+1
!             call pk2f_subREADMATwrp  (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
! 
!          case ( NOP+79 )
!             k0 = st-narg(i)+1
!             call pk2f_subWRITEMAT    (matrs=[(pdata(k),k=k0,st)], status=pdata(k0))
!             st = k0
!             
!          case ( NOP+82 )
!             k0 = st-narg(i)+1
!             call pk2f_subMESHGRIDwrp (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
! 
!          case ( NOP+83 )
! !
...
!             k0 = st-narg(i)+1
!             if ( nsdata == 0 ) then
!                call pk2f_subSORTwrp  (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             else   
!                call pk2f_subSORTwrp  (matrs=[(pdata(k),k=k0,st)], res=pdata(k0), res2=sdata(1))
!                nsused = max(nsused,1_Ikind)
!             end if   
!             st = k0
!                           
!          case ( NOP+84 )
!             k0 = st-narg(i)+1
!             call pk2f_subALLwrp      (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
! 
!          case ( NOP+85 )
!             k0 = st-narg(i)+1
!             call pk2f_subANYwrp      (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
! 
!          case ( NOP+87 )
!             k0 = st-narg(i)+1
!             call pk2f_subRANDIwrp    (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
! 
!          case ( NOP+88 )
!             k0 = st-narg(i)+1
!             call pk2f_subRANDPERMwrp (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
!             
!          case ( NOP+91 )
!             k0 = st-narg(i)+1
!             call pk2f_subCOVwrp      (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
! 
!          case ( NOP+93 )
!             k0 = st-narg(i)+1
!             call pk2f_subCONVSTRwrp  (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             st = k0
! 
!          case ( NOP+94 )
! !
...
!             k0 = st-narg(i)+1
!             if ( nsdata == 0 ) then
!                call pk2f_subREPLACEwrp  (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
!             else   
!                call pk2f_subREPLACEwrp  (matrs=[(pdata(k),k=k0,st)], res=pdata(k0), &
!                                          res2=sdata(1))
!                nsused = max(nsused,1_Ikind)
...
!             else if ( narg(i) == 1 ) then ! e.g. a(m), a(3:2:15), a(:)
!                call pk2f_subEXTRACMAT (a=vars(ivar), res=pdata(st), vindx=pdata(st))
!             else if ( narg(i) == 2 ) then ! e.g. a(m,n), a(:,21), a(:,:)
!                call pk2f_subEXTRACMAT (a=vars(ivar), res=pdata(st-1), vindx=pdata(st-1), &
!                                        vjndx=pdata(st))
!                st = st - 1               
!             end if
!             
! !          case (nop+100)
! !             k0 = st-narg(i)+1
! !             call pk2f_drivertruc1  (matrs=[(pdata(k),k=k0,st)], res=pdata(k0))
! !             st = k0
!          
! !
! !-       Internal error:
...
!=============================================================================================    
   SUBROUTINE pk2Interpreter_EvalExpr_v3 ( handle, vars, answer, flagerr, nanswer ) ! modified (riad)
!============================================================================================= 
   type   (hdl_t),              intent(in out) :: handle
   class  (pk2_t),              intent(in    ) :: vars(:)
   type   (pk2_t), allocatable, intent(   out) :: answer(:)
...
!- local variables --------------------------------------------------------------------------- 
   character(len=* ), parameter   :: HERE = 'pk2Interpreter_EvalExpr_v3'
...
!          case (nop+100)
!             k0 = st-narg(i)+1 ; assignto = k0
!             call pk2f_drivertruc1  (matrs=[(pdata(k),k=k0,st)], res=es)
!             st = k0
...
!                              
         ...
         if ( assignto > 0 ) &
            call pk2_movealloc ( from = res, to = pdata(assignto), movename = .false. )
...
!
   ...
   END SUBROUTINE pk2Interpreter_EvalExpr_v3

!=============================================================================================    
   SUBROUTINE pk2Interpreter_Identifica ( expr, flagerr ) ! modified (riad)
!=============================================================================================    
   character(len=:), allocatable, intent(in    ) :: expr 
   type     (err_t),              intent(in out) :: flagerr
!--------------------------------------------------------------------------------------------- 
...
! 
   ch1 = expr(1:1) ; 
   !id = pk2Interpreter_FindId ( ch1, OperCode )
   Id = util_FindPosInUnsortedList ( OperCode, IONE, NOP, iachar(ch1) )      
...
!
   ch1 = expr(lexpr:lexpr) ; !id = pk2Interpreter_FindId ( ch1, OperCode )
   Id = util_FindPosInUnsortedList ( OperCode, IONE, NOP, iachar(ch1) )
...
!
   ...
   if ( err == 2 ) then   
      !i = pk2Interpreter_FindId ( ch2, OperCode )
      i = util_FindPosInUnsortedList ( OperCode, IONE, NOP, iachar(ch2) )      
...
!=============================================================================================    
   SUBROUTINE pk2Interpreter_Convert ( expr, flagerr ) ! modified (riad)
!=============================================================================================  
   ...
   if ( handle%initialized ) then
      if ( handle%flag%code > 0 ) then
         ...
         if ( handle%noperat > 0 .and. handle%ntokens > 0 ) then  
            call pk2Interpreter_EvalExpr_v3 ( handle, vars, valExpr, flagErr, nAnswer ) 
         else
...
! 
   Id = pk2Interpreter_FindId ( expr(1:p1-1), FuncList ) ; if (Id == 0) return
...
!=============================================================================================
   character(len=*), intent(in out) :: expr
   type     (err_t), intent(in out) :: flagerr
!--------------------------------------------------------------------------------------------- 
...
!  algebraic and logical operators:
   ...
   OperCode(1) = iachar("+"); OperDesc(1) = "Addition"
   
   OperList(2) = "-"        ; StartEnd(2,:) = [t,f] ; ForbidSq(2) = "~*/^\&|:=,;)]}"//OPERS
   OperCode(2) = iachar("-"); OperDesc(2) = "Subtraction"
      
   OperList(3) = "|"        ; StartEnd(3,:) = [f,f] ; ForbidSq(3) = "*/^\&|)]}:=,;"//OPERS
   OperCode(3) = iachar("|"); OperDesc(3) = "Logical OR"
   
   OperList(4) = "-"        ; StartEnd(4,:) = [t,f] ; ForbidSq(4) = "~*/^\&|:=,;)]}"//OPERS
   OperCode(4) = iachar("-"); OperDesc(4) = "Numeric negation"
   
   OperList(5) = "~"        ; StartEnd(5,:) = [t,f] ; ForbidSq(5) ="*/^\&|)]}:,;'"//OPERS
   OperCode(5) = iachar("~"); OperDesc(5) = "Logical negation"
   
   OperList(6) = "*"        ; StartEnd(6,:) = [f,f] ; ForbidSq(6) = "~*/^\&|:=,;)]}"//OPERS
   OperCode(6) = iachar("*"); OperDesc(6) = "Multiplication"
   
   OperList(7) = "/"        ; StartEnd(7,:) = [f,f] ; ForbidSq(7) = "~*/^\&|:=,;)]}"//OPERS
   OperCode(7) = iachar("/"); OperDesc( 7) = "Divide"
   
   OperList(8) = ".*"       ; StartEnd(8,:) = [f,f] ; ForbidSq(8) = "~*/^\&|:=,;)]}"//OPERS
   OperCode(8) = iELEMMUL   ; OperDesc(8) = "Element-wise multiplication"
   
   OperList(9) = "./"       ; StartEnd(9,:) = [f,f] ; ForbidSq(9) = "~*/^\&|:=,;)]}"//OPERS
   OperCode(9) = iELEMDIV   ; OperDesc(9) = "Element-wise divide"
   
   OperList(10) = "&"        ; StartEnd(10,:) = [f,f] ; ForbidSq(10) = "*/^\&|)]}:=,;"//OPERS
   OperCode(10) = iachar("&"); OperDesc(10) = "Logical AND" 
   
   OperList(11) = "\"        ; StartEnd(11,:) = [f,f] ; ForbidSq(11) = "~*/^\&|:=,;)]}"//OPERS
   OperCode(11) = iachar("\"); OperDesc(11) = "Linear system solve (or mldivide())"   
   
   OperList(12) = "^"        ; StartEnd(12,:) = [f,f] ; ForbidSq(12) = "~*/^\&|:=,;)]}"//OPERS
   OperCode(12) = iachar("^"); OperDesc(12) = "Exponentiation"    
   
   OperList(13) = ".^"      ; StartEnd(13,:) = [f,f] ; ForbidSq(13) = "~*/^\&|:=,;)]}"//OPERS
   OperCode(13) = iELEMPOW  ; OperDesc(13) = "Element-wise exponentiation"
   
!  relational operators:

   OperList(14) = "=="      ; StartEnd(14,:) = [f,f] ; ForbidSq(14) = "~*/^\&|)]}:,;"//OPERS
   OperCode(14) = iEQUAL    ; OperDesc(14) = "Equal to (or eq())"     
   
   OperList(15) = "~="      ; StartEnd(15,:) = [f,f] ; ForbidSq(15) = "~*/^\&|)]}:,;"//OPERS
   OperCode(15) = iNOTEQ    ; OperDesc(15) = "Not equal to (or ne())"
   
   OperList(16) = "<"        ; StartEnd(16,:) = [f,f] ; ForbidSq(16) = "~*/^\&|)]}:,;"//OPERS
   OperCode(16) = iachar("<"); OperDesc(16) = "Less than (or lt())"
   
   OperList(17) = ">"        ; StartEnd(17,:) = [f,f] ; ForbidSq(17) = "~*/^\&|)]}:,;"//OPERS
   OperCode(17) = iachar(">"); OperDesc(17) = "Greater than (or gt())"
   
   OperList(18) = "<="      ; StartEnd(18,:) = [f,f] ; ForbidSq(18) = "~*/^\&|)]}:,;"//OPERS
   OperCode(18) = iLESSOREQ; OperDesc(18) = "Less than or equal (or le())"
   
   OperList(19) = ">="      ; StartEnd(19,:) = [f,f] ; ForbidSq(19) = "~*/^\&|)]}:,;"//OPERS
   OperCode(19) = iGREATOREQ; OperDesc(19) = "Greater than or equal (or ge())"
   
!  delimiters:

   OperList(20) = "("       ; StartEnd(20,:) = [t,f] ; ForbidSq(20) = "*/^\&|=,;"//OPERS
   OperCode(20) = iachar("("); OperDesc(20) = "Left parenthesis" 
   
   OperList(21) = ")"       ; StartEnd(21,:) = [f,t] ; ForbidSq(21) = "([{"//ALPHB
   OperCode(21) = iachar(")"); OperDesc(21) = "Right parenthesis"
    
   OperList(22) = ","       ; StartEnd(22,:) = [f,f] ; ForbidSq(22) = "*/^\&|;=,)]}"//OPERS
   OperCode(22) = iachar(","); OperDesc(22) = "Comma delimiter" 
   
   OperList(23) = ";"       ; StartEnd(23,:) = [f,f] ; ForbidSq(23) = "*/^\&|)]}:=,;"//OPERS
   OperCode(23) = iachar(";"); OperDesc(23) = "Semi-colon delimiter"
    
   OperList(24) = "["       ; StartEnd(24,:) = [t,f] ; ForbidSq(24) = "*/^\&|=,;:"//OPERS
   OperCode(24) = iachar("["); OperDesc(24) = "Left bracket (array constructor)"
   
   OperList(25) = "]"       ; StartEnd(25,:) = [f,t] ; ForbidSq(25) = "([{"//ALPHB
   OperCode(25) = iachar("]"); OperDesc(25) = "Right bracket (array constructor)"
   
   OperList(26) = "{"        ; StartEnd(26,:) = [t,f] ; ForbidSq(26) = "*/^\&|=,;:"//OPERS
   OperCode(26) = iachar("{"); OperDesc(26) = "Left brace (not yet used)"
    
   OperList(27) = "}"        ; StartEnd(27,:) = [t,f] ; ForbidSq(27) = "([{"//ALPHB
   OperCode(27) = iachar("}"); OperDesc(27) = "Right brace (not yet used))"
    
!  others:   

   OperList(28) = "'"        ; StartEnd(28,:) = [f,t] ; ForbidSq(28) = "([{~"  //ALPHB ! (04/20)
   OperCode(28) = iachar("'"); OperDesc(28) = "Conjugate transpose (or trans())"
    
   OperList(29) = ".'"       ; StartEnd(29,:) = [f,t] ; ForbidSq(29) = "([{~"  //ALPHB ! (04/20)
   OperCode(29) = iELEMTRANS ; OperDesc(29) = "transpose (or transp())"
    
   OperList(30) = ":"        ; StartEnd(30,:) = [f,f] ; ForbidSq(30) = "~*/^\&|<>:=;]}"
   OperCode(30) = iachar(":"); OperDesc(30) = "Colon operator (or colon())"
    
   OperList(31) = "="        ; StartEnd(31,:) = [f,f] ; ForbidSq(31) = "*/^\&|<>)]}:,;"
   OperCode(31) = iachar("="); OperDesc(31) = "Assignment"
    
   OperList(32) = "!"        ; StartEnd(32,:) = [f,t] ; ForbidSq(32) = "([{~"  //ALPHB
   OperCode(32) = iachar("!"); OperDesc(32) = "Experimental"
!
!- 2) Function names:
!
...
!=============================================================================================
   FUNCTION pk2Interpreter_FindIdStr ( str, ListNames ) result ( Id )
!=============================================================================================   
   character(len=*), intent(in) :: str
...
!=============================================================================================
   FUNCTION pk2Interpreter_FindIdChar1 ( str, ListNames ) result ( Id )
!=============================================================================================   
   character(len=*), intent(in) :: str
